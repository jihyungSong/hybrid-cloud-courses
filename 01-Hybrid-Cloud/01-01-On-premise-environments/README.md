...
해당 실습의 경우, Oregon 리전 (us-west-2) 를 선택 하여 구성 하였습니다. (어떤 리전이든 상관 없습니다)  

VPC 메뉴로 이동 하여, VPC 생성을 시작 합니다.
- VPC 이름 : `on-premise-network`
...
- Subnet 은 총 2개를 생성 합니다.
    * **Subnet 1**
      * 이름 : `on-premise-subnet-01`
      * Availability Zone : 해당 리전의 `첫번째` AZ 를 선택합니다. 해당 예제는 us-west-2 리전에 구성 하였으므로, `us-west-2a` 를 선택 합니다.
      * IPv4 CIDR: `10.10.10.0/24`
    * **Subnet 2**
      * 이름 : `on-premise-subnet-02`
      * Availability Zone : 해당 리전의 `두번째` AZ 를 선택합니다. 해당 예제는 us-west-2 리전에 구성 하였으므로, `us-west-2b` 를 선택 합니다.
      * IPv4 CIDR: `10.10.20.0/24`
...
VPC 는 위에서 생성한 `on-premise-network` VPC 를 선택 합니다.  
...
Route tables 리스트에서 해당 VPC ID (`on-premise-network` VPC 의 ID) 로 검색 하여, VPC 가 생성 될때 자동으로 구성된 기본 Route table 을 선택합니다.  
...
Security groups 리스트에서 해당 VPC ID (`on-premise-network` VPC 의 ID) 로 검색 하여, VPC 가 생성 될때 자동으로 구성된 기본 Security Group 을 선택합니다.  
